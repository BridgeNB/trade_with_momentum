...
def get_top_n(prev_returns, top_n):
    ...
    top_stocks[:] = 0
    for prev_returns_index, row in prev_returns.iterrows():
        n_largest_indexes = row.nlargest(top_n).index
        for index in n_largest_indexes:
            top_stocks.loc[prev_returns_index].loc[index] = 1
        
    return top_stocks.apply(np.int64)
...
def portfolio_returns(df_long, df_short, lookahead_returns, n_stocks):
    ...
    portfolio_returns = df_long * lookahead_returns - df_short * lookahead_returns
...
# I got p-value 0.07 which is a little bit greater than 0.05. This indicates my strategy doest reject the null hypothesis. So this trading strategy may not work. It may project profit by chance. More elaboration needed to reduce the p value.
